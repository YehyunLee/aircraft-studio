...
export default function Preview() {
  ...
  useEffect(() => {
    ...
    const url = `/model-preview.html?modelUrl=${encodeURIComponent(src)}${title ? `&title=${encodeURIComponent(title)}` : ""}`;
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-br from-[#050816] via-[#071032] to-[#07101a] text-white font-sans">
      ...
      <main className="px-3 pb-6">
        {!src ? (
          <div className="p-6 text-center text-white/70">No model provided. Append <code>?src=/models/your.glb</code> — or open Quick Preview in the app.</div>
        ) : (
